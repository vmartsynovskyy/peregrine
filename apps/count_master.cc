...
int main(int argc, char *argv[])
{
  if (argc < 3)
  {
    std::cerr << "USAGE: " << argv[0] << " <data graph> <pattern | #-motifs | #-clique> <# threads> [# workers]" << std::endl;
    return -1;
  }
  ...
  size_t num_workers = argc > 4 ? std::stoi(argv[4]) : 1;
  ...
  Peregrine::Master master = Peregrine::create_master(num_workers);
